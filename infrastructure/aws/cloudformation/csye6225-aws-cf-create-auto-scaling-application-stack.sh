...
netStackName="$netname-csye6225-network"
...
##do
  ##  if [ "$i" == "$netStackName" ] ; then
        ...
        read -p "Enter Bucket Name for Lambda Function: " LambdaBucketName

        read -p "Enter Application Zip Name: " ZipFile

        read -p "Enter Template File Name: " TemplateFile

        echo "Fetching AWS ARN for SSL Certificate"
        
        CertificateArn=$(aws acm list-certificates --certificate-statuses ISSUED --query "CertificateSummaryList[?DomainName=='csye6225-s19-$BucketName.me']"  | jq -r ".[0].CertificateArn")
        
        echo "CertificateArn : $CertificateArn"

        echo "...................... Creating $appStackName ......................"
        printf "......................  Please wait  ...................... \n"
        aws cloudformation create-stack --stack-name $appStackName --template-url https://s3.amazonaws.com/lambda.csye6225-s19-$LambdaBucketName.me/$TemplateFile --capabilities CAPABILITY_NAMED_IAM --parameters ParameterKey=AppName,ParameterValue=$appStackName ParameterKey=NetName,ParameterValue=$netStackName ParameterKey=SSHKeyName,ParameterValue=$SSHKeyName  ParameterKey=AMI,ParameterValue=$AMI ParameterKey=BucketName,ParameterValue=$BucketName ParameterKey=LambdaBucketName,ParameterValue=$LambdaBucketName ParameterKey=ZipFile,ParameterValue=$ZipFile ParameterKey=CertificateArn,ParameterValue=$CertificateArn
